...
app.post('/api/multiple-requests', async (req, res) => {
    try {
        const servers = loadServers();
        const credentials = Buffer.from(`${DEFAULT_USER}:${DEFAULT_CLAVE}`).toString('base64');
...
app.post('/api/status-branch', async (req, res) => {
    ...
    if (!serverUrl || !branchId) {
        return res.status(400).json({ error: 'Faltan parámetros: serverUrl o branchId' });
    }
    try {
        const credentials = Buffer.from(`${DEFAULT_USER}:${DEFAULT_CLAVE}`).toString('base64');
...
app.post('/api/managementinformation', async (req, res) => {
    ...
    try {
        const credentials = Buffer.from(`${DEFAULT_USER}:${DEFAULT_CLAVE}`).toString('base64');
        const fullUrl = serverUrl + managementinformation + branchId + "/servicePoints";
        const response = await fetch(fullUrl, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
...
app.post('/api/servers', (req, res) => {
    try {
        ...
        if (servers.some(s => s.ID === nuevo.ID)) {
            return res.status(400).json({ error: 'Ya existe un servidor con ese ID' });
        }
        servers.push(nuevo);
